...
public class GateMapper extends PepperMapperImpl {
	...
	/**
	 * Parses GATE XML-Document V2/3 with SAX-Parser Salt representation is
	 * constructed successive annotations are divided into nodes and spans with
	 * informations
	 */
	@Override
			...
			DefaultHandler handler = new DefaultHandler() {
				...
				public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
					if (TextWithNodes_TAG.equals(qName)) {
						...
						if (attributes.getLength() > 0) {
							nodeID = attributes.getValue(0).trim();
						} else {
							logger.error("Node {} has no attribute", nodeID);
						}
